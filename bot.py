...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'scan':
        ...
        await query.edit_message_text(
            f"📁 *Department {dept}*\n"
            f"Halaman {page+1}/{total_pages} | Total {total} produk",
            parse_mode='Markdown',
            reply_markup=InlineKeyboardMarkup(keyboard)
        )
        ...
        await query.edit_message_text(
            ...
            "📌 Department: 61, 69, 97\n"
            "📌 Fitur: Scan barcode, Cari produk\n\n"
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == 'WAITING_BARCODE':
        ...
        if result:
            ...
            await update.message.reply_text(
                f"❌ Barcode `{text}` tidak ditemukan.",
                parse_mode='Markdown'
            )
        ...
        cursor.execute(
            "SELECT dept, sku, nama_produk FROM products WHERE nama_produk ILIKE %s OR sku ILIKE %s LIMIT 10",
            (f'%{text}%', f'%{text}%')
        )
